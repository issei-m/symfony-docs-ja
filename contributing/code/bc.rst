...
.. [2] 基本的に行うべきではない。やむを得ない場合はUPGARADEファイルに変更を明記する必要がある。

.. [3] 追加される親インターフェースは、現状のインターフェースに存在しないメソッドを定義してはならない。
...
.. [5] 引数の型は互換性がある、またはより曖昧な型にのみ変更可能。許可されている変更は次の通り:
       ...
       変更元の型           変更先の型
       ===================  =============================================================================
       boolean              `boolean値`_ と等価の\ `スカラー型`_
       string               `string値`_ と等価の\ `スカラー型`_ またはオブジェクト
       integer              `integer値`_ と等価の\ `スカラー型`_
       float                `float値`_ と等価の\ `スカラー型`_
